...
// Magnet relay opens at HIGH
Atm_led magnet, debug;
...
//    magnet.on();
    magnet.blink(2000, 2000, 3).start();
}
...
void gesture_loop() {
    ...
    if (!gesture_error) {
        switch (gesture_data)                                 // When different gestures be detected, the variable 'gesture_data' will be set to different values by paj7620ReadReg(0x43, 1, &gesture_data).
        {
            ...
            case GES_DOWN_FLAG: // REAL: GES_RIGHT_FLAG:
                modbus_set(RIGHT, 1);
                modbus_set(LEFT, 0);
                modbus_set(UP, 0);
                modbus_set(DOWN, 0);
                ...
                break;
            case GES_UP_FLAG: //REAL: GES_LEFT_FLAG:
                modbus_set(RIGHT, 0);
                modbus_set(LEFT, 1);
                modbus_set(UP, 0);
                modbus_set(DOWN, 0);
                ...
                led_feedback_is_showing.on();
                break;
            case GES_RIGHT_FLAG: // REAL: GES_UP_FLAG:
                modbus_set(RIGHT, 0);
                modbus_set(LEFT, 0);
                modbus_set(UP, 1);
                modbus_set(DOWN, 0);
                ...
                led_feedback_is_showing.on();
                break;
            case GES_LEFT_FLAG: // REAL: GES_DOWN_FLAG:
                modbus_set(RIGHT, 0);
                modbus_set(LEFT, 0);
                modbus_set(UP, 0);
                modbus_set(DOWN, 1);
